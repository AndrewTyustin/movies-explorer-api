...
function registerUser(req, res, next) {
  ...
  if (!PASSWORD_REGEX.test(password)) {
    throw new INACCURATE_DATA_ERROR(
  ...
  bcrypt
    ...
    .then(() => res
      .status(201)
      .send({ message: 'Пользователь успешно зарегистрирован на сайте' }))
    .catch((err) => {
      if (err.code === 11000) {
        next(
          new CONFLICT_ERROR(
...
function setCurrentUserInfo(req, res, next) {
  ...
  User.findByIdAndUpdate(
    ...
    .catch((err) => {
      if (err.name === 'CastError') {
        return next(
          new INACCURATE_DATA_ERROR('Передан некорректный id пользователя'),
        );
      }
